...
def test_get_one_post(authorized_client, test_posts):
    ...
    print(res.json())
    post = schemas.PostOut(**res.json()) ## pydantic model is returned
...
def test_create_post_default_published_true(authorized_client, test_posts, test_user):
    res = authorized_client.post("/posts/", json={"title": "Hello123", "content" : "content123"})
    created_post = schemas.Post(**res.json())
    assert res.status_code == 201
    assert created_post.title == "Hello123"
    assert created_post.content == "content123"
    assert created_post.published == True
    assert created_post.owner_id == test_user['id']

def test_unathorized_user_get_create_posts(client, test_posts, test_user):
    res = client.post("/posts/", json={"title": "Hello123", "content" : "content123"})
    assert res.status_code == 401

def test_unathorized_user_delete_post(client, test_posts, test_user):
    res = client.delete(f"/posts/{test_posts[0].id}")
    assert res.status_code == 401

def test_authorized_user_delete_posts_success(authorized_client, test_posts, test_user):
...
def test_update_other_user_post(authorized_client, test_posts, test_user, test_user2):
    data = {
        "title" : "updated title",
        "content" :"updated conten",
        "id" : test_posts[3].id   # 4th item of post is of user2
    }
